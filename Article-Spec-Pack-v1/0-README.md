...
Naturalization applies `10-Final-Prose-Naturalization/Final-Prose-Naturalization-Directive.md` before publication. Editorial QA receives that directive and the full voice profile, and must ground language, rhetoric, structure, and preservation checks in exact excerpts. A clean phrase scan is insufficient. A same-model review is recorded as an independence waiver, not an independent judgment.
...
Visual planning compares the proposed layout with an alternative and omission. A sequential loop, parallel review, branch, and trend convey different relationships. `parallel_review` represents shared context, two isolated reviews, reconciliation, implementation, deferral, actual-diff audit, and a retained result. Renderers reject unsupported label counts and text that cannot fit; they cannot silently drop steps or add a generic telemetry caption. Source and capacity checks do not prove visual quality. Inspect the rendered diagram at article and narrow-screen widths during the host's visual review; state when only source markup was inspected. A diagram cannot be called optimal from one unchecked template.
