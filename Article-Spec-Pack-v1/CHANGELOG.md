...
- Closed a verification gap exposed while resuming the article: a ledger that reports an evidence escalation or omits sources still cited in the article now fails instead of advancing to voice selection.
...
- Added `VISUAL_PLAN` to the enabled controller-hosted model route and every matching provider example. The real correction run exposed that the state existed in the workflow but had no executable route, so active-session automation stopped at a task packet instead of rendering the planned visuals. A pending agent-hosted packet is now durably retired and reminted when an eligible controller route becomes available after an upgrade, and doctor checks every model stage for an automatable route instead of checking research alone.
- Kept source identity stable through post-edit verification. Verification packets now require exactly one source locator per claim, tell the second verifier to preserve the accepted locator for unchanged claims, and reserve qualifications for the wording or supporting excerpt instead of concatenating them into a URL field.
- Recovered same-state directed repairs correctly. When a code-owned finding sends a post-edit ledger back to post-edit verification instead of the workflow's broader article-rewrite destination, the repair context now records that effective destination; runs written by the earlier 3.1 build are normalized from their hash-bound findings during replay.
- Made rendered visuals authoritative before claim locking. The deterministic renderer now replaces model-guessed image blocks in the draft with the manifest's exact public SVG paths and captions, image links are excluded from factual-link locking, and packaging rematerializes canonical visual Markdown before inserting one hash-bound accessible HTML figure. This prevents broken placeholder assets, stale package Markdown, and duplicate diagrams.
- Preserved direct operator amendments through editorial repair. If editorial QA finds a bounded issue in an operator-authored article, the controller now stops with a scoped `amend` instruction instead of silently sending the whole article through a writing model; the amended article then repeats claim verification and editorial QA. This exceptional correction path does not add a routine human gate, so voice selection remains the workflow's only normal pause.
- Made compact comparison tables readable at phone widths. The safe Markdown renderer now records each table's column count, and the publication stylesheet gives three-column tables a fixed responsive layout instead of hiding the decision column behind an unannounced horizontal pan. Wider tables retain the bounded scroll wrapper.
- Stopped console reconstructions from silently dropping planned rows. The deterministic renderer now fits up to seven bounded labels inside the visual, with regression coverage for the request, all four clarifying questions, and the observed recurrence marker used by this correction run.
- Added a safe held-publication visual refresh. Rendered source assets and their manifest are now content-addressed by both the approved plan and their output bytes, so a deterministic renderer upgrade creates new immutable evidence instead of colliding with an older artifact. `render-visuals --refresh` is limited to `PUBLISH_APPROVAL`, keeps the approved plan and article unchanged, records both manifest hashes, and returns the run to `PACKAGE` without reopening the voice gate.
- Classified article-derived live checks by their exact-byte dependency. During a normal Pages rollout, stale article bytes can also carry an old embedded revision or old links; those findings now remain bounded propagation retries, and external links are inspected only after the deployed article matches the approved bytes. Current bytes with broken markup still block immediately.
- Allowed one model to retain multiple hash-approved canary receipts and select the receipt matching the active Codex CLI version. Windows and WSL can now remain independently release-ready when their native CLIs differ, without overwriting or weakening either host's isolated transport evidence.
...
- Closed a stage's bounded repair window when its gate passes, and derived that floor from the last settled gate so runs already recorded recover. POST_EDIT_CLAIM_VERIFICATION and EDITORIAL_QA declare EDIT as their repair state, so their own baselines were never advanced and each could execute only max_attempts times in an entire run: a live run escalated as attempt_window_exhausted reporting two attempts of two with zero rejections. An exhausted-window stop is now also lifted when its window reopens, while the many integrity paths that persist BLOCKED are deliberately never lifted automatically.
- Stopped counting the active host as a routing alternative while automation drives the run. Each content rejection records a route failure, so once every model capable of a stage had produced one rejected output the fallback rule excluded them all and selected a route that execute-stage refuses to perform, which is the outcome that rule exists to prevent.
- Routed a repair by what its findings are about. Every code-owned check at POST_EDIT_CLAIM_VERIFICATION validates the ledger it just produced, yet the stage declares EDIT, so a malformed source URL rewrote the article and regressed citations the ledger had already accepted. The gate-receipt and task-packet schemas already carried `repair_state` on each finding; the verification gates now set it, and both automatic and operator repairs follow it when the findings agree. A receipt written before the field was populated recovers its destination from its own criteria, so a run already stopped at one of these gates routes correctly too.
...
- Left the voice choice as the only pause. A soft review gate the run delegates to policy no longer becomes a second stop: once its bounded repairs are spent the reviewed artifact is accepted and the unresolved findings are recorded on the receipt and in a POLICY_APPROVAL event. Scoped by the workflow's own hard and soft classification, so every code-owned gate still blocks and the voice probe, which is soft but never policy-owned, still waits for the operator.
...
- Let a repair supersede a pending voice decision. A rejected probe leaves a decision recorded against the voice gate, and only a dispatched probe task cleared it, so the repair that the rejection itself opened could never dispatch: the run held for an operator choice over candidates its own gate had refused.
- Replanned instead of stopping when the publication target moves. Both staleness checks told the operator to create and approve a new plan while leaving the run at PUBLISH, where planning is unreachable, so a target repository that advanced for any unrelated reason stopped the run permanently. Nothing has been copied at that point, so the stale plan is dropped and the run returns to approval, where a fresh plan is built against the current head.
- Bound a name both publication recoveries read but never defined. Each raised NameError the first time its branch was reached, and a live run retried one of them twenty-five times without ever reaching the recovery. Their tests passed because they asserted on the controller's source text and matched the very lines that were broken. The suite now runs CPython's own scope analysis over the module, so a name that resolves to a global which does not exist fails the build wherever it appears, including in branches no test reaches.
