...
## August 18, 2026 — Article Flow 2.0 (controller 2.0.7)
...
- Controllers 2.0.1–2.0.7 make installed-release provenance self-contained, forward the configured publication checkout into installed conformance tests, remove source-tree and Unix-home assumptions, retain failed and successful conformance receipts in history, replace the undeclared third-party JSON Schema runtime dependency with the controller's bounded validator for the pack's declared Draft 2020-12 subset, disable cross-host line-ending conversion for protected bytes, isolate nested installer tests from real user launchers, require conformance to exercise the installed launcher from an unrelated directory after the test suite, keep mutable exact-release host status in the host receipts checked by `doctor` rather than freezing it into the implementation snapshot, and separate product/account skill installation from local filesystem discovery without duplicating the workflow. These portability fixes came from immutable Windows/WSL and installed-entrypoint tests rather than source-tree-only assumptions.
