...
## August 18, 2026 — Article Flow 2.0 (controller 2.0.4)
...
- Controllers 2.0.1–2.0.4 make installed-release provenance self-contained, forward the configured publication checkout into installed conformance tests, remove source-tree and Unix-home assumptions, retain failed and successful conformance receipts in history, replace the undeclared third-party JSON Schema runtime dependency with the controller's bounded validator for the pack's declared Draft 2020-12 subset, and disable cross-host line-ending conversion for protected bytes. These portability fixes came from immutable Windows/WSL tests rather than source-tree-only assumptions.
