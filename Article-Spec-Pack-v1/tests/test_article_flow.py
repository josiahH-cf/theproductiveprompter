    ...
    def test_review_regate_same_file_terminal_and_targeted_repairs_are_safe(self):
        ...
        for attempt in range(1, 4):
            af.append_event(directory, run, "TASK_DISPATCHED", "test", {
                ...
                "attempt": attempt,
                "packet": f"tasks/claim_verification-{attempt:02d}.json",
            })
        ...
        # A monotonic packet ordinal is not itself a model execution.  The new
        # repair window therefore starts after zero durable executions here.
        self.assertEqual(run["attempt_baselines"]["CLAIM_VERIFICATION"], 0)
...
class GlobalCommandTests(TemporaryRuntime):
    def test_release_health_checks_both_installed_hosts_and_native_conformance(self):
        ...
        expected_installations = ["windows"] if os.name == "nt" else ["wsl", "windows"]
        expected_conformance = "native-windows" if os.name == "nt" else "wsl"
        self.assertEqual([item["host"] for item in installations["checks"]], expected_installations)
